...
(function($,appPass){
    var $appPassSection           = $( '#spirit-passwords-section' ),
        $newAppPassForm           = $appPassSection.find( '.create-spirit-password' ),
        $newAppPassField          = $newAppPassForm.find( '.input' ),
        $newAppPassButton         = $newAppPassForm.find( '.button' ),
        $appPassTwrapper          = $appPassSection.find( '.spirit-passwords-list-table-wrapper' ),
        $appPassTbody             = $appPassSection.find( 'tbody' ),
        $appPassTrNoItems         = $appPassTbody.find( '.no-items' ),
        $removeAllBtn             = $( '#revoke-all-spirit-passwords' ),
        tmplNewAppPass            = wp.template( 'new-spirit-password' ),
        tmplAppPassRow            = wp.template( 'spirit-password-row' ),
        tmplNotice                = wp.template( 'spirit-password-notice' ),
    ...
    $newAppPassButton.click( function( e ) {
        ...
        $.ajax( {
            url:        appPass.root + appPass.namespace + '/passwords/' + appPass.user_id + '/add',
            method:     'POST',
            beforeSend: function( xhr ) {
    ...
    $appPassTbody.on( 'click', '.delete', function( e ) {
        ...
        $.ajax( {
            url:        appPass.root + appPass.namespace + '/passwords/' + appPass.user_id + '/' + slug,
            method:     'DELETE',
            beforeSend: function( xhr ) {
                xhr.setRequestHeader( 'X-WP-Nonce', appPass.nonce );
            }
    ...
    $removeAllBtn.on( 'click', function( e ) {
        ...
        $.ajax( {
            url:        appPass.root + appPass.namespace + '/passwords/' + appPass.user_id,
            method:     'DELETE',
            beforeSend: function( xhr ) {
                xhr.setRequestHeader( 'X-WP-Nonce', appPass.nonce );
            }
            ...
            if ( parseInt( response, 10 ) > 0 ) {
                ...
                $appPassSection.children( '.new-spirit-password' ).remove();
                $appPassTwrapper.hide();
            }
        } );
    });

    $( document ).on( 'click', '.spirit-password-modal-dismiss', function( e ) {
        e.preventDefault();

        $('.new-spirit-password.notification-dialog-wrap').hide();
    });
